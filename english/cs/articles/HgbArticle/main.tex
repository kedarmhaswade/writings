...
audience are high school students (and laypeople) with aptitude for abstract thinking and an interest in problem-solving.
\end{abstract}
...
A ready-made answer is probably just an Internet search away. Just for fun, however, one should find such an arithmetic expression without searching it on the Web. Give it a try before you read further. Don't worry, the author took quite some time to find it. If you don't have an answer and are restless, skip to the end.

\subsection{Initial Impressions}
This puzzle is surprisingly challenging, especially if one is not well-versed with solving such puzzles. We take arithmetic for granted, but the four basic operations combine with four small numbers to render it tantalizingly difficult. This is so even if you could solve it.
...
Even if we solve the puzzle, we keep wondering if we weren't lucky. If we are about to abandon the search and declare that no solution exists, we seem to lack the confidence that we have indeed examined every expression. 

Most of all, we feel the need of an assistance from a machine that accepts expressions and evaluates them correctly and, well, mechanically. It's a solace that just such an accomplice exists: A computer! 
...
% To our dismay or to our pleasant surprise, ``AI-powered'' computers have started showing signs of a startling but \textit{disembodied} intelligence, but we will save that topic for later.  
...
So, how do we fit this puzzle within the confines of the strict formalism that a computer demands? If only we could examine every expression \dots

\subsection{Some Formal Background}
To develop a perspective, we need to precisely define \textit{expression}, \textit{operator}, and \textit{operand}. An operator has a symbol and describes an operation on a number of operands which are just numbers in our case. Every operation \textit{takes} a number of operands and \textit{produces} a result (or results) that is, in many ways, like its operands--again, numbers in our case. Depending upon how many operands the operation takes, an operator can be classified \textit{unary} (takes one operand), \textit{binary} (takes two operands), etc. This property of an operator is called its \textit{arity}. 
  ...
  \State $T \gets [t_1, t_2, \dots,t_n]$
	\While{T has tokens} \Comment{Start scanning the expression from left}
	  \State $t \gets$\Call{T.next}{}
		\If{$t$ is an operand}
			\State \Call {S.push}{t} \Comment{\textcolor{red}{Returns Error if the stack is full}}
		\ElsIf{$t$ is an operator} \Comment{Defensive programming}
			...
			\\
			\Comment{\textcolor{red}{Returns Error if the stack doesn't have enough operands to pop}}
...
We want to employ a reliable method to \emph{exhaustively} examine all expressions involving four numbers ($1, 3, 4, 6$) and (binary) arithmetic operators ($+, -, \times, \div$). Can postfix notation help? \\

Since there are four operands and each \emph{binary} operator takes two operands and produces a result (which may then pair up with next operand), we must have exactly three binary operators in an expression with seven tokens.
\end{tcolorbox}
...
\begin{arrangement}
  ...
  \begin{tikzpicture}
    ...
    \coordinate (s) at ($(s) + (1.5,0)$);
    \node[fill=red!10, minimum width=18mm, draw=cyan, rectangle] at (s) {$=Error$};
	\end{tikzpicture}
	\\
...
\begin{algorithmic}[2]
  ...
  \ForAll{$p \in P$}
    \ForAll{$t \in T$}
      ...
      \If{$r = g$}
        \State \Output {Bingo! Found a way to make g: $e$}
      \EndIf
    \EndFor
  \EndFor
  \If {$f = false$}
...
Many optimizations are possible. The technique is also extensible to unary operators like factorial. However, algorithm [\ref{alg2}] is of a generic nature and therefore useful in practice.
\subsection{An Implementation}
A Java implementation of algorithm [\ref{alg2}] is available at \cite{Fazlur}. It correctly produces the expressions that solves this and similar problems.
...
The puzzle is simple enough for any curious layperson familiar with basic arithmetic to attempt and enjoy, but it begets some serious computational questions. Every possible expression must be evaluated to find a match. Once a claim for a match is made, one can easily verify if the claim is valid. For four operands, we have to analyze more than $50000$ expressions in the worst case. How many expressions might we have to analyze if the number of operands is 10? Or 20? If there is no expression that evaluates to a given target, is there a trick that lets us \emph{quickly} conclude that without having to analyze every expression? In other words, do we have a more efficient algorithm to solve this puzzle? 
...
The puzzle represents one class of problems in combinatorics. Combinatorics is full of such problems where we devise algorithms to solve simply stated problems of immense complexity. And perhaps that is all we can do with such problems.

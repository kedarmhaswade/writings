...
%%This is Resonance General Article sample
%%download resonance.cls einstein, newton.eps and logo.eps and save it in the folder
%%of  your source file
%%
...
%%txfonts,graphicx,marginnote,array,amsmath,
%%amsfonts,amsthm,amssymb,and geometry 
%%
...
\title{A Combinatorial Puzzle from Arithmetic}
\secondTitle{In Praise of Recreational Mathematics}
...
\authorIntro{\includegraphics[width=2cm]{kedar-pic}\\
Kedar Mhaswade is a computer programmer and an independent researcher. He studied mechanical engineering at the Government College of Engineering in Pune, India and computer science at IIT Chennai (1997). He has worked as a computer programmer at several companies since then. He is interested in logic and foundations of computer science, computer algorithms, functional programming, and mathematics and science education. His \href{https://orcid.org/0009-0004-0436-5661}{ORCID ID} is $0009-0004-0436-5661$.}
%%abstract
\begin{abstract}
...
Consider the following puzzle\footnote{The author first heard it from \href{https://sunilsingh-42118.medium.com/}{Sunil Singh}, a mathematics educator}:
...
The correct answer is probably just an Internet search\footnote{Or a chat with your favorite Large Language Model or agentic AI} away. Just for fun, however, one should find such an arithmetic expression without searching the Web. Try it before you read further. Don't worry, the author took quite some time to find it. If you don't find it and are restless, skip to the end.
...
In our attempts, we may start with heuristics: If we can make an expression using 1, 3, and 4 that evaluates to 4, then we can make 24 by multiplying that expression by 6. But now we have a different puzzle that is just as tricky.
...
%%insert keywords separated by comma in the text, exactly aligned to the required margin text.

\subsection{Some Formal Background}
To develop a perspective, we need to precisely define \emph{expression}, \emph{operator}, and \emph{operand}. 

An operator has a symbol and describes an operation on several operands which are just numbers in our case. Every operation \emph{takes} some operands and \emph{produces} a result (or results) that is, in many ways, like its operands--again, numbers in our case. Depending upon how many operands the operation takes, an operator can be classified \emph{unary} (takes one operand), \emph{binary} (takes two operands), etc. This property of an operator is called its \emph{arity}. Operators can also be \emph{commutative} or \emph{non-commutative}. Results of commutative binary operators do not depend on the \emph{order} of their operands: $a+b=b+a$, whereas those of non-commutative binary operators do: $a\div b \ne b\div a$ 
...
\keywords{algorithms, arithmetic, recreational mathematics, combinatorics, reverse polish notation} 
...
To evaluate expressions, we propose the idea of a \emph{stack}. A stack is an abstract structure that resembles a stack of dishes we often see at restaurants. If we only allow adding and removing a dish to and from the top of the stack, then we must first remove a dish added last. This view makes stack a linear queue whose elements are added and removed only from one of its two ends--the top\footnote{A stack is therefore called a LIFO--Last In First Out--queue}. 
...
\label{alg1}
\begin{algorithmic}[1]
  \State $S \gets \Call{Stack.New}{}$ \Comment{Make a new stack}
  \State $T \gets [t_1, t_2, \dots,t_n]$
	\While{T has tokens} \Comment{Scan the expression from left}
	  \State $t \gets$\Call{T.next}{} \Comment{Get the next token}
		\If{$t$ is an operand}
			\State \Call {S.push}{t}
		\ElsIf{$t$ is an operator} \Comment{Write defensive program}
			\State $od2 \gets$ \Call {S.pop}{ } \Comment{Pop \emph{two} operands}
			\State $od1 \gets$ \Call {S.pop}{ } \Comment{Order matters!}
			\\
			\Comment{\textcolor{red}{Return Error: not enough operands on the stack}}
			\State $r \gets$ \Call {op}{od1, od2} \Comment{Do it: e.g. 2 + 3 and $r \gets 5$}
			\State \Call {S.push}{r}
		\Else 
...
Infix and postfix (or prefix) are, after all, just notation. And it so happens that one can deterministically translate a valid infix expression into postfix and vice versa. 
...
Consider a valid arrangement from Figure (\ref{all-ops-with-one-operand-arr}):
...
Here is how the Algorithm (\ref{alg1}) behaves in that case (scanning proceeds from left to right; stack, initially empty, grows up):

\begin{center}
\begin{longtable}{|c|l|c|l|}
 \hline
    Token & Lines from Listing (\ref{alg1}) & Stack & Tokens Yet to Scan\\[0.5ex]
 \hline\hline
 1 & 5--6 &
 \begin{tikzpicture}[baseline=(current bounding box.center)]
   \draw[fill=green!20] (0,0) rectangle (1.5,0.6);
   \node at (0.75,0.3) {1};
 \end{tikzpicture}
    &
 [4 + 3 + 6 $\times$] 
 ...
 \begin{tikzpicture}[baseline=(current bounding box.center)]
  ...
  \draw[fill=green!20] (0,0.6) rectangle (1.5,1.2);
  \node at (0.75,0.9) {4};
\end{tikzpicture}
    &
 [+ 3 + 6 $\times$] 
...
\begin{algorithmic}[2]
  ...
  \ForAll{$p \in P$}
    \ForAll{$t \in T$}
      \State $e \gets \Call {expr}{p, t}$ \Comment{$e\gets$ from given permutation, tuple}
      \State $r \gets \Call {evalPostfix}{e}$ \Comment{$r\gets$ use algorithm (\ref{alg1})}
      \If{$r = g$}
        \Output {Found g: $e$}
	\State $f = true$
      \EndIf
...
The author describes a surprising application of a technique he learned in graduate school to solve a combinatorial puzzle he encountered on social media years later. Of course, there may be other techniques to solve the puzzle. 
...
%%Use section* for unnumbered sections and \footnote for a normal footnote

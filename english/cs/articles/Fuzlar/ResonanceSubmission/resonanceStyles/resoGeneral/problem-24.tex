...
The correct answer is probably just an Internet search away. Just for fun, however, one should find such an arithmetic expression without searching the Web. Try it before you read further. Don't worry, the author took quite some time to find it. If you don't find it and are restless, skip to the end.
...
\keywords{algorithms, arithmetic, recreational mathematics, permutations, tuples, reverse polish notation} 
	...
	\While{T has tokens} \Comment{Scan the expression from left}
	  \State $t \gets$\Call{T.next}{}
		...
		\ElsIf{$t$ is an operator} \Comment{Write defensive program}
			\State $o2 \gets$ \Call {S.pop}{ } \Comment{Pop \emph{two} operands}
			\State $o1 \gets$ \Call {S.pop}{ } \Comment{Order matters!}
			\\

...
\begin{enumerate}
    ...
    \item Coordinate Systems:
        \begin{enumerate}
            ...
            \item In Polar coordinate system, if $\uvec{e_r}$ is the unit vector in the direction of the radius vector $\vect{r}$ and $\uvec{e_\theta}$ the unit vector in the direction perpendicular to the radius vector, then we get $\vect{r} = r\uvec{e_r}$. Note that $\uvec{e_\theta}$ is not used in this expression, but it will be used often.
            ...
            \item In the vector notation for Spherical Polar coordinates, we have three unit vectors (you can imagine standing on the surface of the sphere):
                \begin{enumerate}
                    \item $\uvec{e_r}$ -- the unit vector along $OP$; this points vertically upward. As in the case of two dimensions, $\vect{r} = r\uvec{e_r}$.

...
\begin{exercise}
    Word count and word count histogram vectors. Suppose an $n$-vector $\boldsymbol{w}$ is the word count vector associated with a document and a dictionary of $n$ words. For simplicity, we will assume that all words in the document appear in the dictionary.
...
\begin{exercise}
    Symptoms vector. A $20$-vector $\boldsymbol{s}$ records whether each of $20$ different symptoms is present in a medical patient, with $\boldsymbol{s}_i=1$ meaning the patient has the symptom and $\boldsymbol{s}_i=0$

...
\begin{frame}
\frametitle{Introducing Calculus Made Easy}
\framesubtitle{Thompson's Treatise for Beginners}
\label{slide:intro1}
\begin{itemize}
\pause
\item These Slides Are Based on Silvanus Thompson's Book, Calculus Made Easy.
\pause
\item This Book Introduces Calculus Simply; Avoids Jargon.
\pause
\item After Quite Some Deliberation, [KM] Chose This Book To Introduce Calculus to Rujuta.
\pause
\item We Also Go on `Excursions' Outside The Book.
\pause
\item Each Chapter Is A Separate Presentation to Keep The Slide Deck Size Manageable.
\pause
\item It Is Supposed to Act Like A ``Ready Reckoner'' (Afterward).
\pause
\item New Words Are Introduced Carefully. Many ``Thought Experiments'' Are Introduced. Please Spend Time on Them.
\end{itemize}
\end{frame}
